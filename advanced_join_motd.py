...
# 7. %since: <date>% 自某日开始至今的日期的绝对值, 输入日期应形如yyyy-mm-dd
        ...
        if not line.startswith('#'):
            ...
            for element in rtext_ori:
                ...
                if element_content == 'day' and daycount_imported:
                    ...
                    rtext_converted.append(rclick(f'§7{element_content.lstrip("7")}§r', f'点击执行 §7{clean(element_content.lstrip("7"))}§r', clean(element_content.lstrip("7"))))
                elif element_content.startswith('8'):
                    rtext_converted.append(rclick(f'§7{element_content.lstrip("8")}§r', f'点击执行 §7{clean(element_content.lstrip("8"))}§r', clean(element_content.lstrip("8")), RAction.suggest_command))
                elif element_content.startswith('a'):
                    rtext_converted.append(rclick(f'§a{element_content.lstrip("a")}§r', f'点击复制 §a{clean(element_content.lstrip("a"))}§r', clean(element_content.lstrip("a")), RAction.copy_to_clipboard))
                elif element_content.startswith('n'):
                    rtext_converted.append(rclick(f'§n{element_content.lstrip("n")}§r', f'点击访问 §n{clean(element_content.lstrip("n"))}§r', clean(element_content.lstrip("n")), RAction.open_url))
                elif element_content.startswith('API:'):
                    rtext_converted.append(access_api(element_content.lstrip('API:').strip()))
                elif element_content.startswith('since:'):
                    rtext_converted.append(get_day(element_content.lstrip('since:').strip()))
                else:
